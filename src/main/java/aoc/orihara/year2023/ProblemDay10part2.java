...
public class ProblemDay10part2 {
...
//            System.out.println("aaaaaaaaaaaaaaa");
        ...
        long result = 0;
        result = countWeebs(map);
        System.out.println("Result:" + result);
    }

    private long countWeebs(List<List<Node>> map) {
        int result = 0;
        ...
        for (List<Node> line : map) {
            ...
            for (Node n : line) {
//                if (switchOn && n.c == '.') {
                if (switchOn && !isPartOfTheLoop(n)) {
                    result++;
                } else
                    // only check what changes in the direction of the search
                    // so only take in consideration the 'down'
                    if ("|7F".indexOf(n.c) != -1 && isPartOfTheLoop(n)) {
                        // dont switch with '-' and 'S'
                        // 'S' is a special case and this works on this example input
                        switchOn = !switchOn;
                    }
            }
        }
    ...
    private boolean isPartOfTheLoop(Node n) {
        return n.left == 2 || n.right == 2 || n.up == 2 || n.down == 2;
    }
